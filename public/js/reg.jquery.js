...
$(function() {
    $("<img />").load(function() {
        var c = function() {
            ...
            if (g < e) {
                f.css({
                    width: $(window).width(),
                    height: "auto",
                    "margin-top": "-" + ((f.height() - $(window).height()) / 2) + "px"
                })
            } else {
                f.css({
        ...
        var b = $('<div class="fullscreen-bg"></div>').css("background-image", "url(../images/reg/36.jpg)").append('<img src="images/reg/36.jpg" />');
        $("body").append(b);
        $(window).on("resize",
        function() {
            c()
        ...
        c()
    }).attr("src","images/reg/36.jpg");
	
	
    $("input, textarea").placeholder();
//    $.getJSON("/api.php/relationInfo",
//    function(a) {
//        if (a.id) {
//            $("<img />").load(function() {
//                var c = function() {
...
//                    }
//                };
//                var d = '<div class="photoby">&lt&nbsp;<a href="http://www.mafengwo.cn/i/' + a.id + '.html" target="_blank">' + a.title + '</a>&nbsp;&gt;&nbsp;by&nbsp;<a href="http://www.mafengwo.cn/u/' + a.author.id + '.html" target="_blank">' + a.author.name + "</a></div>";
//                var b = $('<div class="fullscreen-bg"></div>').css("background-image", "url(" + a.photo + ")").append('<img src="' + a.photo + '" />');
...
if (typeof window.SamsungAccount !== "undefined" && "samsung-life" !== window.PARTNER_AUTH_PLAT) {
    var userId = "";
    window.getSAIDCallback = function(b) {
        var a = JSON.parse(b);
        if (a.mSAHashUId !== "unknown") {
            userId = a.mSAHashUId
        }
    };
    window.SamsungAccount.getSAID("getSAIDCallback");
    setTimeout(function() {
        window.location.href = "/samsung-life/auth/?id=" + userId + "&redirect=" + encodeURIComponent(location.href)
    },
    500)
}
if (typeof window.lives !== "undefined" && "huawei-life" !== window.PARTNER_AUTH_PLAT) {
    window.setAccessToken = function(b) {
        b = JSON.parse(b);
        if (parseInt(b.code) === 200) {
            var c = window.lives.getTokenVerifyUrl();
            var a = "/huawei-life/auth/bind/?accesstoken=" + encodeURIComponent(b.accessToken) + "&source=" + b.source + "&verifyurl=" + encodeURIComponent(c) + "&redirect=" + encodeURIComponent(location.href);
